...
import {GasAbschilleDetailViewComponent} from './view/view.component';
import {GasAbschilleDetailEditComponent} from './edit/edit.component'
import {DevicesService} from "../../../user-service/devicesService";
import {NzMessageService, NzModalService} from "ng-zorro-antd";

@Component({
  selector: 'app-device-gas-abschille-detail',
  templateUrl: './gas-abschille-detail.component.html',
})
export class GasAbschilleDetailComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: '额定制热功率(kW)', type: 'number', index: 'ratedHeatPower' },
    { title: '额定制冷功率(kW)', type: 'number', index: 'ratedCoolPower' },
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: GasAbschilleDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: GasAbschilleDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"gas_abschille")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(GasAbschilleDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"gas_abschille").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
