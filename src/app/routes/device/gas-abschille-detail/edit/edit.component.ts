...
export class GasAbschilleDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      ...
      ratedHeatPower: { type: 'string', title: '额定制热功率', default: 0, minimum: 0,ui: {
        addOnAfter: 'kW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      ratedCoolPower: { type: 'string', title: '额定制冷功率', default: 0, minimum: 0, ui: {
        addOnAfter: 'kW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      ...
      capitalCurve1: { type: 'number', title: '初建成本1',  default: 0, minimum: 0 , ui: {
        validator: val => (!String(val).match(/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/g)) ? [{ keyword: 'required', message: '至多为两位小数' }] : []
      },},
      ...
      numberOrCapacity2: { type: 'number', title: '台数2',  default: 1, minimum: 0 , format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      ...
      numberOrCapacity4: { type: 'number', title: '台数4',  default: 10000, minimum: 0 },
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"gas_abschille")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"gas_abschille").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"gas_abschille").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
