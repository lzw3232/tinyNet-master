...
// @Component({
//   selector: 'app-device-host-detail-edit',
//   templateUrl: './edit.component.html',
// })
// export class HostDetailEditComponent implements OnInit {
...
export class HostDetailEditComponent implements OnInit {
  ...
  schema: SFSchema = {
    properties: {
      ...
      iceRatio: { type: 'string', title: '制冰能效比', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
      ...
      ratedIceElectricPower: { type: 'string', title: '额定制冰电功率', default: 0, minimum: 0, ui: {
        addOnAfter: 'KW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      ...
      iceConsumptionCoefficient: { type: 'string', title: '制冰耗电系数', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
    ...
    required: ['name', 'iceRatio', 'refrigerationRatio', 'life', 'ratedIceElectricPower', 'ratedRefrigeratingElectricPower', 'manufacturer', 'iceConsumptionCoefficient', 'refrigerationConsumptionCoefficient', 
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"host").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"host").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
