...
import {HydroTurbineDetailViewComponent} from "./view/view.component";
import {NzMessageService, NzModalService} from "ng-zorro-antd";
import {HydroTurbineDetailEditComponent} from "./edit/edit.component";
import {DevicesService} from "../../../user-service/devicesService";

@Component({
  selector: 'app-device-hydro-turbine-detail',
  templateUrl: './hydro-turbine-detail.component.html',
})
export class HydroTurbineDetailComponent implements OnInit {
  data : any;
  pi : Number;
  ps : Number;
  total=0;
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: HydroTurbineDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: HydroTurbineDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
            this.modalService.confirm({
              ...
              nzCancelText: 'Cancel',
              nzOnOk:()=>{
                this.devicesService.delete(record.id,"hydro_turbine")
                  .subscribe((res) => {
                    console.log(res);
                    if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .createStatic(HydroTurbineDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"hydro_turbine").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
