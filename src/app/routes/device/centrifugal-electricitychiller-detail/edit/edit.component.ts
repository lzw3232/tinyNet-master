...
@Component({
  selector: 'app-device-centrifugal_electricitychiller-detail-edit',
  templateUrl: './edit.component.html',
})
export class CentrifugalElectricitychillerDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      name: { type: 'string', title: '型号名称' },
      ratedPower: { type: 'string', title: '额定电功率', default: 0, minimum: 0, ui: {
        // widget: 'string',
        addOnAfter: 'kW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      powerFactor: { type: 'string', title: '耗电系数', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      cop: { type: 'string', title: '综合部分负荷性能系数', default: 0, minimum: 0,ui: {
        addOnAfter: 'IPLV',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
    ...
    required: ['name', 'ratedPower', 'powerFactor', 'cop', 'lifeTimeTime', 'manufacturer',
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"centrifugal_electricitychiller")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"centrifugal_electricitychiller").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"centrifugal_electricitychiller").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
