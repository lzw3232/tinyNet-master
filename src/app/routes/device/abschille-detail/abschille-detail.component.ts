...
import {AbschilleDetailViewComponent} from './view/view.component';
import {AbschilleDetailEditComponent} from './edit/edit.component'
import {DevicesService} from "../../../user-service/devicesService";
import {NzMessageService, NzModalService} from "ng-zorro-antd";

@Component({
  selector: 'app-device-abschille-detail',
  templateUrl: './abschille-detail.component.html',
})
export class AbschilleDetailComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: '额定制热烟气功率(kW)', type: 'number', index: 'ratedHeatPower' },
    { title: '额定制冷烟气功率(kW)', type: 'number', index: 'ratedCoolPower' },
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: AbschilleDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: AbschilleDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"abschille")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(AbschilleDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"abschille").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
