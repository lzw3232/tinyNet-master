...
import {PlateHeatDetailViewComponent} from './view/view.component';
import {PlateHeatDetailEditComponent} from './edit/edit.component'
import {DevicesService} from "../../../user-service/devicesService";
import {NzMessageService, NzModalService} from "ng-zorro-antd";

@Component({
  selector: 'app-device-plate-heat-detail',
  templateUrl: './plate-heat-detail.component.html',
})
export class PlateHeatDetailComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: '换热效率（%）', type: 'number', index: 'heatTransferEff' },
    { title: '额定缸套水功率（kw）', type: 'number', index: 'ratedJacketWaterPower' },
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: PlateHeatDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: PlateHeatDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"plate_heat")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(PlateHeatDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"plate_heat").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
