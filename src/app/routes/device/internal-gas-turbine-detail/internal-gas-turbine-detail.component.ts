...
import {InternalGasTurbineDetailViewComponent} from './view/view.component';
import {InternalGasTurbineDetailEditComponent} from './edit/edit.component'
import {DevicesService} from "../../../user-service/devicesService";
import {NzMessageService, NzModalService} from "ng-zorro-antd";

@Component({
  selector: 'app-device-internal-gas-turbine-detail',
  templateUrl: './internal-gas-turbine-detail.component.html',
})
export class InternalGasTurbineDetailComponent implements OnInit {
  data : any;
  pi : Number;
  ps : Number;
  total=0;
...

    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: InternalGasTurbineDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: InternalGasTurbineDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"internal_gas_turbine")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(InternalGasTurbineDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"internal_gas_turbine").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
