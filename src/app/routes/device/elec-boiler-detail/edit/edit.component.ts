...
@Component({
  selector: 'app-device-elec-boiler-detail-edit',
  templateUrl: './edit.component.html',
})
export class ElecBoilerDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      ...
      cop: { type: 'string', title: '制热能效比', default: 0, minimum: 0, ui: {
        // widget: 'string',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
    ...
    required: ['name', 'lifeTime', 'manufacturer', 'ratedPower', 'cop',
      ...
      'numberOrCapacity4', 'capitalCurve4', 'replacementCost4', 'maintainCost'],
  };
  ui: SFUISchema = {
    '*': {
      grid: { span: 12 },
    ...
    '$cop': {grid: { span: 24 }, spanLabel : 5, spanControl : 7},
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"elec_boiler")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
...
save(value: any) {
  ...
  console.log(value);
  if (this.record.id) {
    this.devicesService.update(value,"elec_boiler").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
    ...
    this.devicesService.add(value,"elec_boiler").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
        this.modal.destroy("true");
        this.msgSrv.create('success', `success`);
