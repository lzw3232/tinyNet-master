...
@Component({
  selector: 'app-device-electric-boiler-detail-edit',
  templateUrl: './edit.component.html',
})
export class ElectricBoilerDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      ...
      electricalRating: { type: 'string', title: '额定电功率', default: 0, minimum: 0, ui: {
        // widget: 'string',
        addOnAfter: 'KW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      heatingRatio: { type: 'string', title: '制热能效比', default: 0, minimum: 0, ui: {
        // widget: 'string',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
    ...
    required: ['name', 'life', 'manufacturer', 'electricalRating', 'heatingRatio',
      ...
      'capacity4', 'initialCost4', 'replacementCost4', 'maintenanceCost'],
  };
  ui: SFUISchema = {
    '*': {
      grid: { span: 12 },
    ...
    '$heatingRatio': {grid: { span: 24 }, spanLabel : 5, spanControl : 7},
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"electric_boiler")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
...
save(value: any) {
  ...
  console.log(value);
  if (this.record.id) {
    this.devicesService.update(value,"electric_boiler").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
    ...
    this.devicesService.add(value,"electric_boiler").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
        this.modal.destroy("true");
        this.msgSrv.create('success', `success`);
