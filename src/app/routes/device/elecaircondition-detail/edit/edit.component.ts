...
@Component({
  selector: 'app-device-elecaircondition-detail-edit',
  templateUrl: './edit.component.html',
})
export class ElecairconditionDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      name: { type: 'string', title: '型号名称' },
      copIce: { type: 'string', title: '制冰能效比', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
      ...
      ratedIcePower: { type: 'string', title: '额定制冰电功率', default: 0, minimum: 0, ui: {
        addOnAfter: 'KW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      rateCoolPower: { type: 'string', title: '额定制冷电功率', default: 0, minimum: 0, ui: {
        addOnAfter: 'KW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      manufacturer: { type: 'string', title: '制造商', maxLength: 140 },
      icePowerFactor: { type: 'string', title: '制冰耗电系数', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
    ...
    required: ['name', 'copIce', 'copCool', 'lifeTime', 'ratedIcePower', 'rateCoolPower', 'manufacturer', 'icePowerFactor', 'coolPowerFactor',
  ...
  ui: SFUISchema = {
    ...
    '$coolPowerFactor': {grid: { span: 24 }, spanLabel : 5, spanControl : 7},
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"elecaircondition")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"elecaircondition").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"elecaircondition").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
