...
@Component({
  selector: 'app-device-electricitychiller-detail-edit',
  templateUrl: './edit.component.html',
})
export class ElectricitychillerDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      ...
      cop: { type: 'string', title: '综合部分负荷性能系数(IPLV)', default: 0, minimum: 0,ui: {
        addOnAfter: 'IPLV',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
    ...
    required: ['name', 'ratedPower', 'powerFactor', 'cop', 'lifeTime', 'manufacturer',
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"electricitychiller")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"electricitychiller").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"electricitychiller").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
