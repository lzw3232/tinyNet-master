...
import {WasteHeatBoilerDetailViewComponent} from './view/view.component';
import {WasteHeatBoilerDetailEditComponent} from './edit/edit.component'
import {DevicesService} from "../../../user-service/devicesService";
import {NzMessageService, NzModalService} from "ng-zorro-antd";

@Component({
  selector: 'app-device-waste-heat-boiler-detail',
  templateUrl: './waste-heat-boiler-detail.component.html',
})
export class WasteHeatBoilerDetailComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: '效率（%）', type: 'number', index: 'efficiency' },
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: WasteHeatBoilerDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: WasteHeatBoilerDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"waste_heat_boiler")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(WasteHeatBoilerDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"waste_heat_boiler").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
