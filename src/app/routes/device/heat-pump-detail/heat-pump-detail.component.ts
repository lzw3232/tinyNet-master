...
@Component({
  selector: 'app-device-heat-pump-detail',
  templateUrl: './heat-pump-detail.component.html',
})
export class HeatPumpDetailComponent implements OnInit {
  data : any;
  pi : Number;
  ps : Number;
  total=0;
    ...
    { title: '制冷耗电系数(%)', type: 'number', index: 'refrigerationConsumptionCoefficient' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: HeatPumpDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: HeatPumpDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"heat_pump")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(HeatPumpDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"heat_pump").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
