...
export class CentrifugalDetailComponent implements OnInit {
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: CentrifugalDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: CentrifugalDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"centrifugal")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(CentrifugalDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"centrifugal").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
