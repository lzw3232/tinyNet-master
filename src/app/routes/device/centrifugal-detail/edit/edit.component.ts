...
@Component({
  selector: 'app-device-centrifugal-detail-edit',
  templateUrl: './edit.component.html',
})
export class CentrifugalDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"centrifugal")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"centrifugal").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
