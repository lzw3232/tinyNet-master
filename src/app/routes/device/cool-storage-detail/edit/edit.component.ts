...
@Component({
  selector: 'app-device-cool-storage-detail-edit',
  templateUrl: './edit.component.html',
})
export class CoolStorageDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      name: { type: 'string', title: '型号名称' },
      efficiencycharge: { type: 'string', title: '蓄冰效率', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
        validator: val => ((Number(val) > 100) ? [{ keyword: 'required', message: '请输入100以内的数' }] : [])
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      efficiencydischarge: { type: 'string', title: '放冰效率', default: 0, minimum: 0, ui: {
        addOnAfter: '%',
        placeholder: '请输入数字',
        validator: val => ((Number(val) > 100) ? [{ keyword: 'required', message: '请输入100以内的数' }] : [])
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      maxchargerate: { type: 'string', title: '最大蓄冰倍率', default: 0, minimum: 0,ui: {
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      maxdischargerate: { type: 'string', title: '最大放冰倍率', default: 0, minimum: 0, ui: {
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"cool_storage")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"cool_storage").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"cool_storage").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
