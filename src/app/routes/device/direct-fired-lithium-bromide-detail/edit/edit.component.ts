...
export class DirectFiredLithiumBromideDetailEditComponent implements OnInit {
  ...
  schema: SFSchema = {
    properties: {
      ...
      ratedHeatingPower: { type: 'string', title: '额定制热功率', default: 0, minimum: 0,ui: {
        addOnAfter: 'kW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      ratedCoolingPower: { type: 'string', title: '额定制冷功率', default: 0, minimum: 0, ui: {
        addOnAfter: 'kW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
      ...
      numberOfUnits2: { type: 'number', title: '台数2',  default: 1, minimum: 0 , format: 'regex', pattern: '^\\d+(\\.\\d+)?$'},
    ...
    'ratedHeatingPower', 'ratedCoolingPower', 'life', 'manufacturer',
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"direct_fired_lithium_bromide")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"direct_fired_lithium_bromide").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"direct_fired_lithium_bromide").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
