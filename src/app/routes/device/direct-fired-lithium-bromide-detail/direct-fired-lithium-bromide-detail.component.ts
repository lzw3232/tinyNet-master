...
export class DirectFiredLithiumBromideDetailComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: '额定制热功率(kW)', type: 'number', index: 'ratedHeatingPower' },
    { title: '额定制冷功率(kW)', type: 'number', index: 'ratedCoolingPower' },
    ...
    { title: '制造商', index: 'manufacturer' },
    {
      ...
      buttons: [
        { text: '<i class="anticon anticon-file-text"></i>', type: 'static',
          component: DirectFiredLithiumBromideDetailViewComponent,
        },
        { text: '<i class="anticon anticon-edit"></i>',
          type: 'static', component: DirectFiredLithiumBromideDetailEditComponent,
        ...
        { text: '<i class="anticon anticon-delete"></i>',
          click: (record, _modal, comp) => {
                this.modalService.confirm({
                  ...
                  nzCancelText: 'Cancel',
                  nzOnOk:()=>{
                    this.devicesService.delete(record.id,"direct_fired_lithium_bromide")
                      .subscribe((res) => {
                        console.log(res);
                        if(res["errno"]=="0"){
  ...
  add() {
    this.modal
      .create(DirectFiredLithiumBromideDetailEditComponent, { i: { id: 0 } })
      .subscribe(res=>{
        if(res){
          this.getlist(this.pi);
        }
  ...
  getlist(pi){
    this.devicesService.list(pi,this.ps,this.val,"direct_fired_lithium_bromide").subscribe((res)=>{
      console.log(res);
      if(res["errno"]=="0"){
