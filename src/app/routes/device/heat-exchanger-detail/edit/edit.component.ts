...
@Component({
  selector: 'app-device-heat-exchanger-detail-edit',
  templateUrl: './edit.component.html',
})
export class HeatExchangerDetailEditComponent implements OnInit {
  i : any;
  record: any = {};
  schema: SFSchema = {
    properties: {
      name: { type: 'string', title: '型号名称' },
      efficiency: { type: 'string', title: '换热效率', default: 0, minimum: 0, ui: {
        // widget: 'string',
        addOnAfter: '%',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      ratedPower: { type: 'string', title: '额定缸套水功率', default: 0, minimum: 0, ui: {
        addOnAfter: 'kW',
        placeholder: '请输入数字',
      }, format: 'regex', pattern: '^0$|^([1-9][0-9]*)$|^((0|([1-9][1-9]*))\.[0-9]+)$'},
      ...
      numberOrCapacity2: { type: 'number', title: '台数2',  default: 10, minimum: 0 },
      capitalCurve2: { type: 'number', title: '初建成本2',  default: 10, minimum: 0 },
      replacementCost2: { type: 'number', title: '更新成本2',  default: 10, minimum: 0 },
      maintainCost2: { type: 'number', title: '运维成本2',  default: 10, minimum: 0 },
      numberOrCapacity3: { type: 'number', title: '台数3',  default: 100, minimum: 0 },
      capitalCurve3: { type: 'number', title: '初建成本3',  default: 100, minimum: 0 },
      replacementCost3: { type: 'number', title: '更新成本3',  default: 100, minimum: 0 },
      maintainCost3: { type: 'number', title: '运维成本3',  default: 100, minimum: 0 },
      numberOrCapacity4: { type: 'number', title: '台数4',  default: 1000, minimum: 0 },
      capitalCurve4: { type: 'number', title: '初建成本4',  default: 1000, minimum: 0 },
      replacementCost4: { type: 'number', title: '更新成本4',  default: 1000, minimum: 0 },
  ...
  ngOnInit(): void {
    ...
    if (this.record.id) {
      ...
      this.devicesService
        .select(this.record.id,"heat_exchanger")
        .subscribe(res => {
          console.log(res);
          if(res["errno"]=="0"){
  ...
  save(value: any) {
    ...
    console.log(value);
    if (this.record.id) {
      this.devicesService.update(value,"heat_exchanger").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
      ...
      this.devicesService.add(value,"heat_exchanger").subscribe((res)=>{
        console.log(res);
        if(res["errno"]=="0"){
          this.modal.destroy("true");
          this.msgSrv.create('success', `success`);
